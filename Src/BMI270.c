...
/*
 * SENSOR INIT DATA ARRAY (Because bosh decide it needs it. The first byte 0x5e is the register address where it needs to be written into. )
 */
uint8_t bmi270_config_file[] = {
   0x5e, 0xc8, 0x2e, 0x00, 0x2e, 0x80, 0x2e, 0x3d, 0xb1, 0xc8, 0x2e, 0x00, 0x2e, 0x80, 0x2e, 0x91, 0x03, 0x80, 0x2e, 0xbc,
...
uint8_t BMI270_Init(BMI270 *imu, SPI_HandleTypeDef *SPI_Handle, GPIO_TypeDef *csPinBank, uint16_t csPin){
	...
	uint8_t status,init_status =0;
	uint8_t chip_id_check;
	uint8_t init_buff_rx[sizeof(bmi270_config_file)]; // receiver buffer for loading config file. Must be same size because in full duplex SPI.
	...
	*/
	status += BMI270_WriteRegister(imu, BMI270_REG_PWR_CONF, 0x00);
	HAL_Delay(10);

	/*
	 * Prepare config load config_file by writing value 0x00.
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_INIT_CTRL, 0x00);

	/*
	 * Perform the burst write to BMI270_REG_INIT_DATA.
	 */
	HAL_GPIO_WritePin(imu->csPinBank, imu->csPin, GPIO_PIN_RESET);
	status += (HAL_SPI_TransmitReceive(imu->SPI_Handle, bmi270_config_file, init_buff_rx, sizeof(bmi270_config_file), HAL_MAX_DELAY) == HAL_OK);
	HAL_GPIO_WritePin(imu->csPinBank, imu->csPin, GPIO_PIN_SET);
	HAL_Delay(10);
	/*
	 * Complete config load by writing value 1 in BMI270_REG_INIT_CTRL.
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_INIT_CTRL, 0x01);
	HAL_Delay(10);
	/*
	 * Check for correct initialization status.
	 */

	/*BMI270_REG_INTERNAL_STATUS bit (3-0)
	 ...
	 *0x05 nvm_error Internal error while accessing NVM
	 *0x06 start_up_error Internal error while accessing NVM and Initialization error
	 *0x07 compat_error Compatibility error
	 */
	status += BMI270_ReadRegister(imu, BMI270_REG_INTERNAL_STATUS, &init_status);
	HAL_Delay(10);
	init_status=(init_status && 0x1111);
	if(init_status!=0x1){
		return status=0;
	}

	//==============Config device for normal mode==============//
	 ...
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_PWR_CTRL, 0x06);
	HAL_Delay(10);
	 ...
	 *
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_ACC_CONF, 0xA9);
	HAL_Delay(10);


	/* BMI270_REG_GYR_CONF default 0xA9
	 *
	 * Bit 3-0: gyr_odr (output data rate). Default 100 Hz.
	 * Bit 5-4: gyr_bwp (The Gyroscope bandwidth coefficient defines the 3 dB cutoff frequency of the low pass filter for the sensor data). Default 0x2.
	 * Bit 6: gyr_noise_perf (0 power optimized, 1 performance optimized).
	 * Bit 7: gyr_filter_perf (0 power optimized, 1 performance optimized).
	 *
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_GYR_CONF, 0xE9);
	HAL_Delay(10);
	 ...
	 *
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_ACC_RANGE, 0x00); //+-2g
	HAL_Delay(10);


	/* BMI270_REG_GYR_RANGE
	 * Bit 2-0: gyr_range (0x00 range_2000
	 *
	 */
	status += BMI270_WriteRegister(imu, BMI270_REG_GYR_RANGE, 0x02); //+-2000 rad/s
	HAL_Delay(10);


	status +=BMI270_WriteRegister(imu,BMI270_REG_INT1_IO_CTRL,0x0A);
	HAL_Delay(10);

	//status +=BMI270_WriteRegister(imu,BMI270_REG_INT2_IO_CTRL,0x0A);
	//HAL_Delay(10);

	status +=BMI270_WriteRegister(imu,BMI270_REG_INT_MAP_DATA,0x04);
	HAL_Delay(10);
	...
	*Bit 2: Fast power up enable. 0x00 disable, 0x01 enabled.
	*/
	status += BMI270_WriteRegister(imu, BMI270_REG_PWR_CONF, 0x44);
	HAL_Delay(10);

	//================== Set Conversion Constants==========================//

	//imu->accConversion =
	imu->accConversion = 1/(float)16384*GRAVITY_EARTH;
	imu->gyrConversion = 1/(float)65.536*0.01745329252;

	// Set accelerometer Tx buffer for DMA.
	imu->AccGyrTxBuff[0] = BMI270_REG_ACC_X_LSB|0x80;

	// Set gyroscope Tx buffer for DMA.
	//imu->gyrTxBuff[0] = BMI270_REG_GYR_X_LSB|0x80;
...
uint8_t BMI270_ReadAccelerometer(BMI270 *imu) {
	...
	/* Convert to m/s^2 */
	imu->acc_g[0] = imu->accConversion*accX;
	imu->acc_g[1] = imu->accConversion*accY;
	imu->acc_g[2] =	imu->accConversion*accZ;
...
uint8_t BMI270_Read_GyrRegisters(BMI270 *imu) {
	...
	uint8_t txBuf[8] = {(BMI270_REG_GYR_X_LSB | 0x80), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00}; /* Register addr, 6 bytes data */
	uint8_t rxBuf[8];
...
uint8_t BMI270_ReadAcc_DMA(BMI270 *imu){
	...
	if(HAL_SPI_TransmitReceive_DMA(imu->SPI_Handle, imu->accTxBuff, imu->accRxBuff, 8) ==HAL_OK){

		imu->readingAccFlag=1;
		return 1;
...
void BMI270_Read_Acc_DMA_Complete(BMI270 *imu){
	...
	/* Form signed 16-bit integers */
	int16_t accX = (int16_t) ((imu->accRxBuff[3] << 8) | imu->accRxBuff[2]);
	int16_t accY = (int16_t) ((imu->accRxBuff[5] << 8) | imu->accRxBuff[4]);
	int16_t accZ = (int16_t) ((imu->accRxBuff[7] << 8) | imu->accRxBuff[6]);

	/* Convert to m/s^2 */
	imu->acc_g[0] = imu->accConversion*accX;
	imu->acc_g[1] = imu->accConversion*accY;
	imu->acc_g[2] = accZ/ (float) 8192;
...
uint8_t BMI270_ReadGYR_DMA(BMI270 *imu){
	...
	if(HAL_SPI_TransmitReceive_DMA(imu->SPI_Handle, imu->gyrTxBuff, imu->gyrRxBuff, 8) ==HAL_OK){

		imu->readingGyrFlag =1;
		return 1;
...
void BMI270_Read_Gyr_DMA_Complete( BMI270 *imu){
	...
	/* Form signed 16-bit integers */
	int16_t gyrX = (int16_t) ((imu->gyrRxBuff[3] << 8) | imu->gyrRxBuff[2]);
	int16_t gyrY = (int16_t) ((imu->gyrRxBuff[5] << 8) | imu->gyrRxBuff[4]);
	int16_t gyrZ = (int16_t) ((imu->gyrRxBuff[7] << 8) | imu->gyrRxBuff[6]);
...
uint8_t BMI270_ReadAccGyr_DMA(BMI270 *imu){
	...
	if(HAL_SPI_TransmitReceive_DMA(imu->SPI_Handle,imu->AccGyrTxBuff,imu->AccGyrRxBuff,14) == HAL_OK){

		imu->readingAccGyrFlag=1;
		return 1;
	}
	else{
...
void BMI270_ReadAccGyr_DMA_Complete(BMI270 *imu){
	...
	HAL_GPIO_WritePin(imu->csPinBank,imu->csPin,GPIO_PIN_SET);
	imu->readingAccGyrFlag=0;

	/* Form signed 16-bit integers */
	int16_t accX = (int16_t) ((imu->AccGyrRxBuff[3] << 8) | imu->AccGyrRxBuff[2]);
	int16_t accY = (int16_t) ((imu->AccGyrRxBuff[5] << 8) | imu->AccGyrRxBuff[4]);
	int16_t accZ = (int16_t) ((imu->AccGyrRxBuff[7] << 8) | imu->AccGyrRxBuff[6]);

	/* Form signed 16-bit integers */
	int16_t gyrX = (int16_t) ((imu->AccGyrRxBuff[9] << 8) | imu->AccGyrRxBuff[8]);
	int16_t gyrY = (int16_t) ((imu->AccGyrRxBuff[11] << 8) | imu->AccGyrRxBuff[10]);
	int16_t gyrZ = (int16_t) ((imu->AccGyrRxBuff[13] << 8) | imu->AccGyrRxBuff[12]);
...
//This was from Bosh API i dodn't know how it works but it works. Delete th earth gravity constants that is multiplied in return value in order to get g's.

float lsb_to_mps2(float g_range, uint8_t bit_width)
{
    double power = 2;

    float half_scale = (float)((pow((double)power, (double)bit_width) / 2.0f));

    return ( g_range) / half_scale;
}


static float lsb_to_dps( float dps, uint8_t bit_width)
{
    double power = 2;

    float half_scale = (float)((pow((double)power, (double)bit_width) / 2.0f));

    return (dps / (half_scale));
}
